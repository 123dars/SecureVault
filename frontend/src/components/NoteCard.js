...
export default function NoteCard({ title, content, color = 'slate', onCopy, onDelete }) {
  ...
  return (
    <div className={`group relative bg-[#111827]/60 backdrop-blur-md p-5 rounded-3xl border shadow-xl transition-all duration-300 flex flex-col border-slate-700/50 hover:border-slate-600/60 h-full`}>
      ...
      <h3 className="mb-4 text-lg font-bold text-white leading-tight">{title}</h3>
      <div className={`relative bg-[#0B0F19]/70 px-4 py-3 rounded-xl border border-slate-700/50 flex-grow mb-4 flex flex-col transition-all duration-300`}>
        {showContent ? (
          ...
          <div className="flex-grow flex items-center justify-center py-4">
            <span className="text-sm font-mono text-slate-500 tracking-[0.3em] uppercase">
              • Encrypted •
            </span>
          </div>
        )}
      ...
      <div className="grid grid-cols-3 gap-2 mt-auto">
        ...
        <button
          onClick={onDelete}
          className="flex items-center justify-center p-2.5 rounded-xl border border-rose-500/20 bg-rose-500/10 text-rose-500 hover:bg-rose-600 hover:text-white hover:border-rose-500 transition-all duration-200"
        >
          <Trash2 size={15} />
        </button>
      </div>
